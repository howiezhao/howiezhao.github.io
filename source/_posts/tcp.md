...
TCP 拥塞控制算法（TCP congestion control algorithm）包括 3 个主要部分：
...
同 UDP 一样，首部包括**源端口号**、**目的端口号**以及**检验和字段（checksum field）**。除此之外，还包含以下字段：
...
总结以上，TCP 连接的组成包括：一台主机上的缓存、变量和与进程连接的套接字，以及另一台主机上的另一组缓存、变量和与进程连接的套接字。
